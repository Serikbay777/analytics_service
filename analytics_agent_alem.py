...
# -*- coding: utf-8 -*-
"""
AI Агент для аналитики музыкальных данных на LangGraph с Alem AI
"""
...
# Создаем LLM с инструментами для Alem AI
def create_agent(api_key: str, model: str = "gpt-4o"):
    """Создает агента с Alem AI LLM и инструментами"""
...
# Создаем граф
def create_graph(api_key: str, model: str = "gpt-4o"):
    """Создает LangGraph граф агента"""
    
    llm_with_tools = create_agent(api_key, model)
...
def run_agent(query: str, api_key: str, model: str = "gpt-4o", thread_id: str = "default"):
    """
    Запускает агента с запросом
    
    Args:
        query: запрос пользователя
        api_key: API ключ Alem AI
        model: модель (gpt-4o, gpt-4o-mini, claude-3-5-sonnet-20241022)
        thread_id: ID треда для сохранения истории
    
    Returns:
        Ответ агента
    """
    app = create_graph(api_key, model)
...
# Интерактивный режим
def interactive_mode(api_key: str, model: str = "gpt-4o"):
    """Интерактивный режим общения с агентом"""
    
    print("=" * 80)
    print("🤖 AI АГЕНТ АНАЛИТИКИ МУЗЫКАЛЬНЫХ ДАННЫХ (Alem AI)")
    print("=" * 80)
    print(f"\n📡 Используется модель: {model}")
    print(f"🌐 API endpoint: https://llm.alem.ai/v1")
    print("\nДоступные команды:")
    print("  - Введите ваш вопрос на русском или английском")
    print("  - 'exit' или 'quit' для выхода")
    print("  - 'clear' для очистки истории")
    print("  - 'model <name>' для смены модели")
    print("\nДоступные модели:")
    print("  • gpt-4o (по умолчанию)")
    print("  • gpt-4o-mini")
    ...
    while True:
        try:
            ...
            # Запускаем агента
            app = create_graph(api_key, current_model)
            config = {"configurable": {"thread_id": thread_id}}
...
if __name__ == "__main__":
    ...
    if not api_key:
        print("❌ Ошибка: установите переменную окружения ALEM_API_KEY")
        print("   export ALEM_API_KEY='sk-BYXPF09hIOPGYsKGeswMoQ'")
        exit(1)
    
    # Получаем модель из переменной окружения или используем по умолчанию
    model = os.getenv("ALEM_MODEL", "gpt-4o")
