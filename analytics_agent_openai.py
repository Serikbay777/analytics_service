...
# -*- coding: utf-8 -*-
"""
AI Агент для аналитики музыкальных данных на LangGraph с OpenAI
"""
...
# from langgraph.checkpoint.memory import MemorySaver  # Не используем для стабильности
...
# Создаем LLM с инструментами для OpenAI
def create_agent(api_key: str, model: str = "gpt-4o"):
    """Создает агента с OpenAI LLM и инструментами"""
...
def create_graph(api_key: str, model: str = "gpt-4o"):
    ...
    # Компилируем граф без памяти (для стабильности)
...
def run_agent(query: str, api_key: str, model: str = "gpt-4o"):
    """
    ...
    Args:
        ...
        model: модель (gpt-4o, gpt-4o-mini, gpt-4-turbo)
...
def interactive_mode(api_key: str, model: str = "gpt-4o"):
    ...
    print("=" * 80)
    print("🤖 AI АГЕНТ АНАЛИТИКИ МУЗЫКАЛЬНЫХ ДАННЫХ (OpenAI)")
    print("=" * 80)
    ...
    print("\nДоступные модели:")
    print("  • gpt-4o (по умолчанию, самая мощная)")
    print("  • gpt-4o-mini (быстрая и дешевая)")
    print("  • gpt-4-turbo")
    print("\nПримеры запросов:")
    ...
    while True:
        try:
            ...
            # Запускаем агента
            response = run_agent(user_input, api_key, current_model)
